...
static SDL_HitTestResult SDLCALL hit_test(SDL_Window *window,
...
#if RESIZE_FROM_TOP
  } else if (pt->x > resize_border && pt->x < w - controls_width &&
             pt->y < resize_border) {
...
top:
  ...
  case SDL_MOUSEMOTION:
    SDL_PumpEvents();
    while (SDL_PeepEvents(&event_plus, 1, SDL_GETEVENT, SDL_MOUSEMOTION,
                          SDL_MOUSEMOTION) > 0) {
  ...
  case SDL_MOUSEWHEEL:
    ...
    // Use -x to keep consistency with vertical scrolling values (e.g.
    // shift+scroll)
  ...
  case SDL_FINGERMOTION:
    SDL_PumpEvents();
    while (SDL_PeepEvents(&event_plus, 1, SDL_GETEVENT, SDL_FINGERMOTION,
                          SDL_FINGERMOTION) > 0) {
  ...
  case SDL_APP_DIDENTERFOREGROUND:
    rencache_invalidate();
    lua_pushstring(L, e.type == SDL_APP_WILLENTERFOREGROUND
                          ? "enteringforeground"
                          : "enteredforeground");
    return 1;
...
static const char *window_opts[] = {"normal", "minimized", "maximized",
                                    "fullscreen", 0};
...
static int f_set_window_mode(lua_State *L) {
  ...
  SDL_SetWindowFullscreen(window_renderer.window,
                          n == WIN_FULLSCREEN ? SDL_WINDOW_FULLSCREEN_DESKTOP
                                              : 0);
  if (n == WIN_NORMAL) {
    SDL_RestoreWindow(window_renderer.window);
  }
  if (n == WIN_MAXIMIZED) {
    SDL_MaximizeWindow(window_renderer.window);
  }
  if (n == WIN_MINIMIZED) {
    SDL_MinimizeWindow(window_renderer.window);
  }
  return 0;
}
...
static int f_list_dir(lua_State *L) {
  ...
  WIN32_FIND_DATAW fd;
  HANDLE find_handle = FindFirstFileExW(wpath, FindExInfoBasic, &fd,
  ...
  do {
    ...
    len = WideCharToMultiByte(CP_UTF8, 0, fd.cFileName, -1, mbpath,
...
static struct f_type_names fs_names[] = {
    {0xef53, "ext2/ext3"}, {0x6969, "nfs"},       {0x65735546, "fuse"},
    {0x517b, "smb"},       {0xfe534d42, "smb2"},  {0x52654973, "reiserfs"},
    {0x01021994, "tmpfs"}, {0x858458f6, "ramfs"}, {0x5346544e, "ntfs"},
    {0x0, NULL},
};
...
static int f_mkdir(lua_State *L) {
  ...
  free(wpath);
#else
  int err = mkdir(path, S_IRUSR | S_IWUSR | S_IXUSR | S_IRGRP | S_IXGRP |
                            S_IROTH | S_IXOTH);
#endif
  if (err < 0) {
    lua_pushboolean(L, 0);
...
static int f_fuzzy_match(lua_State *L) {
  ...
  // If true match things *backwards*. This allows for better matching on
  // filenames than the above function. For example, in the lite project,
  // opening "renderer" has lib/font_render/build.sh as the first result, rather
  // than src/renderer.c. Clearly that's wrong.
  ...
  while (strTarget >= str && ptnTarget >= ptn && *strTarget && *ptnTarget) {
    while (strTarget >= str && *strTarget == ' ') {
      strTarget += increment;
    }
    while (ptnTarget >= ptn && *ptnTarget == ' ') {
      ptnTarget += increment;
    }
...
static void *api_require(const char *symbol) {
  static const lua_function_node nodes[] = {
    ...
    LUA_VERSION_NUM == 503 || LUA_VERSION_NUM == 504
      U(addlstring),
      U(addstring),
      U(addvalue),
      U(argerror),
...
static int f_load_native_plugin(lua_State *L) {
  char entrypoint_name[512];
  entrypoint_name[sizeof(entrypoint_name) - 1] = '\0';
  int result;
  ...
  snprintf(entrypoint_name, sizeof(entrypoint_name), "luaopen_lite_xl_%s",
           basename);
  ...
  *(void **)(&ext_entrypoint) = SDL_LoadFunction(library, entrypoint_name);
  if (!ext_entrypoint) {
    ...
    *(void **)(&entrypoint) = SDL_LoadFunction(library, entrypoint_name);
    if (!entrypoint)
      return luaL_error(
          L, "Unable to load %s: Can't find %s(lua_State *L, void *XL)", name,
          entrypoint_name);
    result = entrypoint(L);
  } else {
  ...
  if (!result)
    return luaL_error(L, "Unable to load %s: entrypoint must return a value",
                      name);
...
static int f_path_compare(lua_State *L) {
  ...
  /* If a path separator is present in the name after the common part we
     consider the entry like a directory. */
  ...
  for (i = offset, j = offset; i <= len1 && j <= len2; i++, j++) {
    if (path1[i] == 0 || path2[j] == 0) {
      if (cfr < 0)
        cfr = 0; // The strings are equal
      if (!same_len) {
        cfr = (path1[i] == 0);
      }
    } else if (isdigit(path1[i]) && isdigit(path2[j])) {
      size_t ii = 0, ij = 0;
      while (isdigit(path1[i + ii])) {
        ii++;
      }
      while (isdigit(path2[j + ij])) {
        ij++;
      }
